...
class ShortPokerGameFactory(GameFactory):
    ...
    def create_game(self, players: List[PlayerServer]):
        ...
        return ShortPokerGame(
            self._blind,
            id=game_id,
            game_players=GamePlayers(players),
            event_dispatcher=ShortPokerGameEventDispatcher(game_id=game_id, logger=self._logger),
            deck_factory=DeckFactory(lowest_rank),
            score_detector=ShortPokerScoreDetector(lowest_rank)
        )


class ShortPokerGameEventDispatcher(GameEventDispatcher):
